...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let randcat= [1,6,16,27,37,18,10][Math.floor(Math.random()*7)];
...
let selectedQuestions = []; // Stores the questions the teacher picks

// 1. Fetch ALL questions from a topic so the teacher can see them
async function loadQuestionsForSelection(topicId) {
    const file = `${topicId}.json`;
    const container = document.getElementById('questions-container');
    ...
    try {
        ...
        container.innerHTML = `<h3>Select questions from: ${topic_list[topicId]}</h3>`;
        
        questions.forEach((q, index) => {
            const qDiv = document.createElement('div');
            qDiv.className = 'selection-item';
            qDiv.innerHTML = `
                <label>
                    <input type="checkbox" onchange='toggleQuestion(${JSON.stringify(q)}, this.checked)'>
                    ${index + 1}. ${q.questionText || "Question Content"} 
                </label>
            `;
            container.appendChild(qDiv);
        });

        container.innerHTML += `<br><button onclick="startTeacherExam()">Start Exam with Selected Questions</button>`;
        
    } catch (err) {
        console.error("Error loading questions for selection", err);
    }
}

// 2. Add or remove question from the custom list
...
// 3. Launch the exam using the manually picked list
function startTeacherExam() {
    ...
    isExam = true;
    allQuestions = [...selectedQuestions]; // Set global questions to our selection
    currentQuestionIndex = 0;
