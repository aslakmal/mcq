schoolId||alertbox("Missing school ID in URL","error"),document.getElementById("addStudentBtn").href="student_register.html?school="+schoolId;let cachedStudents=[],selectedStudentIds=new Set,currentPage=1,pageSize=50,searchDebounceTimer=null;function generateClassOptions(){var e=document.getElementById("classList"),t=["A","B","C","D","E"];let n="";for(let e=1;e<=11;e++)for(const a of t)n+=`<option value="${e}${a}"></option>`;e.innerHTML=n}function restoreSearchState(){var e=sessionStorage.getItem("search_"+schoolId)||"",t=sessionStorage.getItem("class_"+schoolId)||"";document.getElementById("searchInput").value=e,document.getElementById("classInput").value=t}async function initApp(){generateClassOptions(),restoreSearchState();try{await syncEngine.initIndexedDB(),await loadFromIndexedDBDirect(),navigator.onLine&&(await syncEngine.syncStudents(),await loadFromIndexedDBDirect())}catch(e){console.warn("Network / Sync init skipped (Offline Mode):",e)}}function loadFromIndexedDBDirect(){return new Promise(t=>{syncEngine.getLocalData("students").then(e=>{e&&0<e.length&&((cachedStudents=e).forEach(e=>selectedStudentIds.add(e.id||e.key)),renderStudentTable(cachedStudents)),t()}).catch(()=>t())})}function renderStudentTable(e){var t=document.getElementById("studentTableBody");const n=document.getElementById("searchInput").value.toLowerCase().trim(),a=document.getElementById("classInput").value.toLowerCase().trim();var s,d,o,l,e=e.filter(e=>{var t=!n||e.fullName&&e.fullName.toLowerCase().includes(n)||e.id&&e.id.toLowerCase().includes(n),e=!a||e.class&&e.class.toLowerCase().includes(a);return t&&e});0===e.length?(t.innerHTML=`<tr><td colspan="6" class="empty-state">${n||a?"No matching students found.":"No students registered yet."}</td></tr>`,renderPagination(0,0),updateSelectionCounter()):(s=e.length,d=Math.ceil(s/pageSize),o=((currentPage=(currentPage=currentPage>d?d:currentPage)<1?1:currentPage)-1)*pageSize,l=Math.min(o+pageSize,s),e=e.slice(o,l),t.innerHTML=e.map(e=>{var t=e.photoUrl?getFullCloudinaryUrl(e.photoUrl):"img/user.svg",n=e.id||e.key;return`
  ...
  `}).join(""),document.getElementById("bulkScoreModal").style.display="flex",await fetchAndUpdateModalScores())}async function fetchAndUpdateModalScores(){var t=document.getElementById("scoreSemesterSelect").value,n=document.getElementById("scoreSubjectSelect").value;for(const s of document.querySelectorAll(".student-score-field")){const d=s.getAttribute("data-id");let e=null;var a=await getStudentFromIDB(d);null===(e=a&&a.exam&&a.exam["s"+t]&&void 0!==a.exam["s"+t][n]?a.exam["s"+t][n]:e)&&(a=cachedStudents.find(e=>(e.id||e.key)===d))&&a.exam&&a.exam["s"+t]&&void 0!==a.exam["s"+t][n]&&(e=a.exam["s"+t][n]),s.value=null!==e?e:""}}async function submitBulkScores(){const a=document.getElementById("scoreSemesterSelect").value,s=document.getElementById("scoreSubjectSelect").value;var e=document.querySelectorAll(".student-score-field");const d={};let o=0;const l=Date.now(),c=firebase.auth().currentUser?firebase.auth().currentUser.uid:"system";if(e.forEach(e=>{const t=e.getAttribute("data-id");var n,e=e.value.trim();""!==e&&(e=Number(e),n=`schools/${schoolId}/students/`+t,d[n+`/z/s${a}/`+s]=e,d[n+"/A"]=firebase.database.ServerValue.TIMESTAMP,d[n+"/B"]=c,(n=cachedStudents.find(e=>(e.id||e.key)===t))&&(n.exam||(n.exam={}),n.exam["s"+a]||(n.exam["s"+a]={}),n.exam["s"+a][s]=e,n.updatedAt=l,n.updatedBy=c,syncEngine.saveToIDB("students",n)),o++)}),0===o)alertbox("Please enter a score for at least one student.","error");else try{navigator.onLine&&await db.ref().update(d),alertbox(`Successfully saved/updated scores for ${o} student(s).`,"success"),closeModal("bulkScoreModal")}catch(e){alertbox("Failed to save scores: "+e.message,"error")}}function openBulkClassModal(){0===getSelectedStudents().length?alertbox("Please select at least one student to change class.","error"):(document.getElementById("newClassInput").value="",document.getElementById("bulkClassModal").style.display="flex")}async function submitBulkClassChange(){const n=document.getElementById("newClassInput").value.trim().toUpperCase();if(n){var e=getSelectedStudents();if(0===e.length)alertbox("No students selected.","error");else{const a={},s=Date.now(),d=firebase.auth().currentUser?firebase.auth().currentUser.uid:"system";e.forEach(e=>{var t=e.id||e.key,t=`schools/${schoolId}/students/`+t;a[t+"/h"]=n,a[t+"/A"]=firebase.database.ServerValue.TIMESTAMP,a[t+"/B"]=d,e.class=n,e.updatedAt=s,e.updatedBy=d,console.log(e),syncEngine.saveToIDB("students",e)});try{navigator.onLine&&await db.ref().update(a),renderStudentTable(cachedStudents),alertbox(`Updated ${e.length} student(s) to Class ${n}.`,"success"),closeModal("bulkClassModal")}catch(e){alertbox("Failed to update classes: "+e.message,"error")}}}else alertbox("Please enter a valid class name.","error")}function closeModal(e){document.getElementById(e).style.display="none"}async function openIdCardModalByKey(t,e){const n=cachedStudents.find(e=>(e.id||e.key)===t);if(n){var a=e?e.innerHTML:"";e&&(e.disabled=!0,e.innerHTML="Loading");try{var s=await getSetting();const i=MODULE_FIELD_MAPS.students;document.getElementById("cardSchoolFront").innerText=s.schoolName||"SCHOOL",document.getElementById("cardSchoolFront").innerText=s.schoolName||"SCHOOL",document.getElementById("cardPhoto").src=n.photoUrl?getFullCloudinaryUrl(n.photoUrl):"img/user.svg",document.getElementById("cardName").innerText=n.fullName||"";var d=s.frontFields||["i","b","h"],o=s.backFields||["e","f","d","g"],l=document.getElementById("frontCardBody"),c=(l&&(l.innerHTML=d.map(e=>{var t=i[e];return t?`
        <div class="info-row">
